...
extern void leopard_finish_suspend(unsigned long cpu_state);
typedef void (*finish_suspend_t)(unsigned long);
...
/*
  ...
  */
int asoc_finish_suspend(unsigned long cpu_state)
{
...
//	int ddr_prio;
	...
	if (pmic_suspend_ops && pmic_suspend_ops->set_wakeup_src) {
	    ...
	    if(judge_adapter_type != NULL)
	    {
	        ...
	        else 
	        {
	            ...
	            case ADAPTER_TYPE_WALL_PLUGIN:
	                /*wall 插入，且充满电*/
	                wakeup_mask = (WAKEUP_SRC_ONOFF_SHORT | WAKEUP_SRC_VBUS_IN | WAKEUP_SRC_WALL_IN | WAKEUP_SRC_ALARM);
	                wakeup_src = (WAKEUP_SRC_ONOFF_SHORT | WAKEUP_SRC_VBUS_IN);				
	                break;
	            ...
	            case ADAPTER_TYPE_USB_ADAPTER_PLUGIN:
	                /*usb adapter 插入*/
	                wakeup_mask = (WAKEUP_SRC_ONOFF_SHORT | WAKEUP_SRC_VBUS_IN | WAKEUP_SRC_WALL_IN | WAKEUP_SRC_ALARM);
	                wakeup_src = (WAKEUP_SRC_ONOFF_SHORT | WAKEUP_SRC_WALL_IN);
	                break;
	            case ADAPTER_TYPE_USB_WALL_PLUGIN:
	                /*usb adapter & Wall adatper 同时插入*/
	                wakeup_mask = (WAKEUP_SRC_ONOFF_SHORT | WAKEUP_SRC_VBUS_IN | WAKEUP_SRC_WALL_IN | WAKEUP_SRC_ALARM);
	                wakeup_src = (WAKEUP_SRC_ONOFF_SHORT);				
	                break;
	            default :
	                /*something wrong*/
	    ...
	    wakeup_src |= WAKEUP_SRC_ONOFF_LONG ;
		wakeup_mask |= WAKEUP_SRC_ONOFF_LONG;
