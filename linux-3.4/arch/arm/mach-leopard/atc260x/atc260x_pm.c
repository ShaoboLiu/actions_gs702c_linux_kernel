...
static void atc260x_prepare_suspend(int mode)
{
    ...
    atc260x_set_access_mode(atc260x_pm.atc260x, 
        ATC260X_SPI_ACCESS_MODE_DIRECT);
}
...
//	 atc260x_reg_write(atc260x_pm.atc260x, atc2603_PMU_SYS_CTL9, (unsigned short) (resume_func_phy>>16));
    ...
    while(i<timeout_cnt)
    {
        ...
        mdelay(1);
        if((atc260x_reg_read(atc260x_pm.atc260x, atc2603_PMU_SYS_CTL3) & PMU_SYS_CTL3_EN_S2) != 0)
        {
            break;
        }
    }
    ...
    pr_debug("[ATC260X PM] PMU_SYS_CTL0:%x, PMU_SYS_CTL3:%x, PMU_SYS_CTL5:%x\n", 
        atc260x_reg_read(atc260x_pm.atc260x, atc2603_PMU_SYS_CTL0),
