...
//	printk("ddr_size 0x%x, fb_size 0x%x\n", asoc_ddr_size, asoc_fb_size);
...
void __init leopard_reserve(void)
{
    ...
    if(!is_qb_kernel) {
      ...
      asoc_ion_data.heaps[0].size = asoc_fb_size;
			...
			printk("pfn asoc_ion_start: %d, asoc_ion_size: %d\n", asoc_ion_start>>12, asoc_ion_size);
			int ret = dma_declare_contiguous(&(asoc_ion_device.dev), asoc_ion_size,
						  asoc_ion_start, asoc_ion_start+asoc_ion_size);
			if(ret != 0)
			{
				printk("dma_declare_contiguous failed to reserve mem base: 0x%08x, size: %d\n",
					asoc_ion_start, asoc_ion_size);
			}
			else
			{
				printk("dma_declare_contiguous successed to reserve mem base: 0x%08x, size: %d\n",
					asoc_ion_start, asoc_ion_size);
			}
